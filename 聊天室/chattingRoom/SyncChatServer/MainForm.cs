...
namespace SyncChatServer
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            msgBox.HorizontalScrollbar = true;
            btn_Stop.Enabled = false;
            onlineBox.HorizontalScrollbar = true;
            SetServerIPAndPort();
        }
        ...
        private delegate void delOnlineDelegate(string message);
        /// <summary>
        /// 在在线框（lst_Online)中添加其他客户端信息
        /// </summary>
        /// <param name="userName"></param>
        private void DelOnline(string userName)
        {
            if (onlineBox.InvokeRequired)
            {
                delOnlineDelegate d = new delOnlineDelegate(DelOnline);
                onlineBox.Invoke(d, new object[] { userName });
            }
            else
            {
        ...
        private void ReceiveData(object userState)
        {
            ...
            while (isNormalExit == false)
            {
                ...
                {
                    if (isNormalExit == false && user.client.Connected)
                    {
                ...
                switch(splitString[0])
                {
                    case "Login":
                        ...
                        //onlineBox.Items.Add(splitString[1]);
                        AddOnline(splitString[1]);
                        SendToAllClient(user,receiveString);
                        break;
                    case "Logout":
                        SendToAllClient(user,receiveString);
                       // onlineBox.Items.Remove(splitString[1]);
                        RemoveUser(user);
                        return;
                    case "Talk":
    ...
    private void SendToAllClient(User user, string message)
        ...
        private void onlineBox_SelectedIndexChanged(object sender, EventArgs e)
        {
